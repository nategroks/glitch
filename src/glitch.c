...
static void print_entropy_footer(int img_gap) {
    ...
    const int max_bar_cells = 16;
    int available_for_bar = available / 3; /* keep the bar compact so the suffix stays close */
    int bar_cells = available_for_bar / cell_width;
    if (bar_cells < 4) bar_cells = 4;
    if (bar_cells > max_bar_cells) bar_cells = max_bar_cells;
    if (bar_cells * cell_width > available) {
        bar_cells = available / cell_width;
