...
mkdir -p "$REPO_DIR/variants" "$REPO_DIR/noise" "$REPO_DIR/encryption" "$SRC_DIR"
cd "$REPO_DIR"
...
# Stats: up to 14, choose from
# distro,kernel,uptime,mem,host,user,shell,cpu,ip,ip4,ip6,pub4,pub6,dns,ntp,fs,disk,ports,entropy
STATS=distro,kernel,uptime,mem,ip4,pub4,disk,ports,cpu,entropy

# Optional: always pull a specific image URL (e.g., copied from lynx)
# IMAGE_URL=https://example.com/square.png

# Fast mode: skip network fetches and public IP/DNS/NTP stats (GLITCH_FAST=1 also works)
# FAST=0
EOF
fi
...
if ! ls "$VARIANT_DIR"/*.png >/dev/null 2>&1; then
  if [ -x "$REPO_DIR/glitch" ]; then
    "$REPO_DIR/glitch" --fetch-only >/dev/null 2>&1 || true
  fi
fi
...
TLDR_PAGE="$DOCS_DIR/tldr-glitch.md"
...
if [ -x "$SCRIPT_DIR/setup-tldr.sh" ]; then
  "$SCRIPT_DIR/setup-tldr.sh" --page "$TLDR_PAGE" --quiet
  echo "[+] Installed TLDR page via scripts/setup-tldr.sh"
else
  echo "[*] tldr/tealdeer helper missing; left TLDR page at: $TLDR_PAGE"
fi

# 8. Ensure repo bin is on PATH
if ! grep -Fq "$REPO_DIR" "$HOME/.bashrc" 2>/dev/null; then
  echo "export PATH=\"$REPO_DIR:\$PATH\"" >> "$HOME/.bashrc"
  echo "[+] Added glitch repo to PATH in ~/.bashrc"
fi
